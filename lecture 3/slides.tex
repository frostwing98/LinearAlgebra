...
that appear in the vector equations.
...
\]
\begin{enumerate}[a.]
\item Determine whether $\{\vect v_1,\vect v_2,\vect v_3\}$ is linearly independent.
...
\]
\begin{itemize}
\item $x_1$ and $x_2$ are basic variables, and $x_3$ is free.
\item Each nonzero value of $x_3$ determines a nontrivial solution of (1).
...
To find a linear dependence relation among $\mathbf{v_1}, \mathbf{v_2}, \mathbf{v_3}$, completely row reducing the augmented matrix gives
\[
...
\begin{itemize}
  \item Thus $x_1=2x_3$, $x_2=-x_3$, and $x_3$ is free. 
  \item Choose, for example, $x_3=5$. 
...
\begin{itemize}
  \item Substituting the values into the vector equation gives
\end{itemize}
...
to a nontrivial solution of}}} $A\vect x=\vect 0$.
...
if and only if $\vect v$ is not the zero vector $\vect 0$}}.
...
\smallskip
\begin{itemize}
  \item The example 4 generalizes to any set $\{\vect u,\vect v,\vect w\}$ in $\mathbb{R}^3$ with $\vect u$ and $\vect v$ linearly independent.
...
\begin{itemize}
  \item For each $\vect x\in\R^n$, $T(\vect x)$ is computed as $A\vect x$, where $A$ is an $m\times n$ matrix. 
  \item For simplicity, we {\color{lectureblue}{denote such a \textit{matrix transformation} by}}  $x \mapsto Ax$
  \item Observe that the domain of $T$ is $\mathbb{R}^m$ when $A$ has $n$ columns and the codomain of $T$ is $\mathbb{R}^m$ when each column of $A$ has $m$ entries.
...
say that multiplication by $A$ transforms $\vect{x}$ into $\vect{b}$ and transforms $\vect{u}$ into the zero vector. See Figure below.
...
\item Any $\vect x$ whose image under $T$ is $\vect b$ must satisfy equation (1).
\begin{itemize}
\item From (2), it is clear that equation (1) has a unique solution.
\item So there is exactly one $\vect x$ whose image is $\vect b$.
\end{itemize}
\end{enumerate}
...
$\vect c=T(\vect x)$ for some $\vect x\in\R^2$. 
...
% Reference page 27
\begin{frame}{Shear Transformation（剪切變換）}
...
\begin{frame}{Reflections: Table 1 Continued}
  \begin{figure}
  \centering
  \includegraphics[width=\textheight]{table2.png}
\end{figure}
\sourcepage{105}
\end{frame}
...
\begin{frame}{Reflections: Table 1 Continued}
 \begin{figure}
  \centering
  \includegraphics[height=\textheight]{table3.png}
\end{figure}
  \sourcepage{106}
\end{frame}
...
\begin{frame}{Table 4: Projections}
   \begin{figure}
  \centering
  \includegraphics[height=\textheight]{table6.png}
\end{figure}
